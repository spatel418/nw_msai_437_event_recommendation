...
Ensure the `data/` directory contains the required files. The core input files are:
- `illinois_business_mapping.json` — Yelp venue data (from Yelp dataset)
- `illinois_user_recommendations.json` — Collaborative filtering output (generated by `notebooks/yelp_illinois_collaborative_filtering.ipynb`)

The remaining files (`event_embeddings_v3.npy`, `user_event_recommendations_v3.json`, etc.) are generated by the pipeline and can be rebuilt using the "Update Events" button in the admin view.
